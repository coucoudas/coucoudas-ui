...
const LAYOUTS = import.meta.glob("/src/_test/app/**/layout.tsx") as Record<
  string,
  () => Promise<{ default: PageComponent }>
>;
const COMPONENTS = import.meta.glob("/src/_test/app/**/[a-z[]*.tsx") as Record<
  string,
  () => Promise<{ default: PageComponent }>
>;
    ...
    const loadRoutes = async () => {
      const routeItems: RouteItem[] = await Promise.all(
        Object.keys(COMPONENTS).map(async (key) => {
          const path = key
            .replace(/\/src\/_test\/app|page|\.tsx$/g, "")
            .replace(/\[\.{3}.+\]/, "*")
            .replace(/\[(.+)\]/, ":$1");
